...
export abstract class Resource {
  ...
  public static findSection(
    ...
    // Normalization function to create a "slug" from a string.
    // This allows for robust matching of heading fragments by:
    // 1. Converting to lowercase.
    // 2. Replacing whitespace with hyphens.
    // 3. Removing any non-alphanumeric characters (except hyphens and underscores).
    // For example, "My Awesome Heading!" becomes "my-awesome-heading".
    ...
    return (
      resource.sections.find(s => {
        // A section is a match if the fragment from the link (in its raw
        // or normalized form) is present in the section's list of valid
        // identifiers. This handles both heading slugs and block IDs.
        // The `linkableIds` are generated by the parser and contain all
        // possible ways to reference this section.
        return (
          s.linkableIds.includes(fragment) ||
          s.linkableIds.includes(normFragment)
        );
      }) ?? null
    );
  }
